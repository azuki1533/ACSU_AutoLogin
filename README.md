...
### iOSのSafariで「サービスプロバイダを特定できませんでした」等のエラーが出る場合

iOSのSafariを使用している場合、プライバシー保護機能によって認証システム（SSO）に必要なCookieの受け渡しがブロックされ、ログインページから進めなくなることがあります。この現象が発生した場合は、以下の手順で設定を変更してください。

1.  iPhone/iPadの **「設定」** アプリを開く。
2.  **「Safari」** をタップする。
3.  「プライバシーとセキュリティ」の項目にある **「サイト越えトラッキングを防ぐ」** を **オフ** にする。
4.  Safariを一度完全に終了（タスクキル）してから、再度ログインを試す。
